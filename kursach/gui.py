...
class EventNotifierGUI:
    ...
    def create_gui(self):
        ...
        self.message_entry = tk.Text(self.root, font=("Arial", 12), width=45, height=10)
        ...
        self.calendar = Calendar(self.root, selectmode='day', date_pattern='yyyy-mm-dd', locale='uk')
    ...
    def add_user(self):
        ...
        name_entry = tk.Entry(new_user_window, width=25, font=("Arial", 12))
        name_entry.pack()

        tk.Label(new_user_window, text="Електронна пошта", bg="#dff0d8").pack()
        email_entry = tk.Entry(new_user_window, width=25, font=("Arial", 12))
        email_entry.pack()

        tk.Label(new_user_window, text="Номер телефону", bg="#dff0d8").pack()
        phone_entry = tk.Entry(new_user_window, width=25, font=("Arial", 12))
        phone_entry.pack()

        contact_var = tk.StringVar(value="email")
        tk.Radiobutton(new_user_window, text="Email", variable=contact_var, value="email", bg="#dff0d8").pack()
        tk.Radiobutton(new_user_window, text="Телефон", variable=contact_var, value="phone", bg="#dff0d8").pack()

        tk.Button(new_user_window, text="Зберегти", font=("Arial", 12), bg="#4CAF50", fg="white",
    ...
    def create_event(self):
        ...
        tk.Label(event_window, text="Назва події:", font=("Arial", 12), bg="#dff0d8").pack(pady=10)
        event_name_entry = tk.Entry(event_window, font=("Arial", 12), width=40)
        event_name_entry.pack(pady=10)

        tk.Label(event_window, text="Час події:", font=("Arial", 12), bg="#dff0d8").pack(pady=10)
        time_entry = tk.Entry(event_window, font=("Arial", 12), width=40)
        time_entry.pack(pady=10)

        tk.Label(event_window, text="Введіть опис події:", font=("Arial", 12), bg="#dff0d8").pack(pady=10)
        event_text = tk.Text(event_window, font=("Arial", 12), width=40, height=6)
        ...
        def save_event():
            ...
            try:
                ...
                self.show_custom_message("Помилка", "Невірний формат часу.", "red")
                return

            self.db.execute_query("INSERT INTO events (user_id, event_date, event_name, event_description) VALUES (?, ?, ?, ?)",
                                  (user_id, event_datetime, event_name, event_description))
            self.show_custom_message("Готово", "Подію додано.", "green")
            event_window.destroy()

        tk.Button(event_window, text="Зберегти", font=("Arial", 12), bg="#4CAF50", fg="white",
